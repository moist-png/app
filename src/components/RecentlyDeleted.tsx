...
export const RecentlyDeleted: React.FC<RecentlyDeletedProps> = ({
  ...
  const fetchDeletedItems = () => {
    const now = Date.now();
    setDeletedReports(getAllRawReports(orgId).filter(r => r.deletedAt && (now - r.deletedAt < SEVEN_DAYS_IN_MS)));
    setDeletedReadings(getAllRawChlorophyllReadings(orgId).filter(r => r.deletedAt && (now - r.deletedAt < SEVEN_DAYS_IN_MS)));
    setDeletedSites(getAllRawSites(orgId).filter(s => s.deletedAt && (now - s.deletedAt < SEVEN_DAYS_IN_MS)));
    setDeletedJobs(getAllRawJobs(orgId).filter(j => j.deletedAt && (now - j.deletedAt < SEVEN_DAYS_IN_MS)));
    setDeletedDailyRisks(getAllRawDailyRisks(orgId).filter(r => r.deletedAt && (now - r.deletedAt < SEVEN_DAYS_IN_MS)));
  };

  React.useEffect(() => {
    if (isOpen) fetchDeletedItems();
  }, [isOpen]);

  const handleRecover = (type: string, id: string) => {
    onRecover(type, id); // App.tsx handles recovery + state refresh
    fetchDeletedItems();  // Refresh the modal list
  };
