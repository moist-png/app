...
export interface Organization {
  id: string;
  name: string;
  slug: string; // e.g. "smith-arborists" — URL-safe unique identifier
  createdAt: number;
  active: boolean;
}
...
export interface OrgInviteCode {
  ...
  code: string;
  createdAt: number;
  createdBy: string; // user id of the org_admin who created it
}
...
export interface User {
  ...
  email: string;
  name: string;
  role: 'super_admin' | 'org_admin' | 'user' | 'guest';
  orgId: string | null; // null only for super_admin
  createdAt: number;
  lastLogin: number;
}
